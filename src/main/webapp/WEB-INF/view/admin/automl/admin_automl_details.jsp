											...
											<table class="table table-bordered table-striped">
												...
												<tbody>
													<c:forEach var="data" varStatus="s" items="${project.dataSets}">
														<tr data-href="${contextPath}/admin/automl/details/${f:u(project.id)}">
															<td>${f:h(data.name)}</td>
															<td class="text-center">
															<c:if test="${data.meta == null and !data.imported}">
																<span class="label label-danger">Unknown</span>
															</c:if>
															<c:if test="${data.meta != null and !data.imported}">
																<span class="label label-warning">Unload</span>
															</c:if>
															<c:if test="${data.meta != null and data.imported}">
					...
					<div class="col-md-6">
						<div class="box box-primary">
							<div class="box-header with-border">
								...
								<div class="btn-group pull-right">
									<la:link href="runml" styleClass="btn btn-success btn-xs ${f:h(editableClass)}">
										<em class="fa fa-plus"></em>
										Run
									</la:link>
								</div>
