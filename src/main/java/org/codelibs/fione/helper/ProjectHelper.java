...
public class ProjectHelper {
    ...
    private final String projectJsonName = "project.json";

    private final Gson gson = H2oApi.createGson();
    ...
    protected DataSet[] parseDataSets(final FessConfig fessConfig, final MinioClient minioClient, final String id) {
        ...
        for (final Result<Item> result : minioClient.listObjects(fessConfig.getStorageBucket(), prefix, false)) {
            try {
                ...
                final String[] values = objectName.split("/");
                if (values.length == 4) {
                    final DataSet dataSet = createDataSet(fessConfig, minioClient, id, values[3]);
                    list.add(dataSet);
                }
            } catch (final Exception e) {
    ...
    private DataSet createDataSet(final FessConfig fessConfig, final MinioClient minioClient, final String projectId, final String name) {
        final String datasSetId = Base64.encodeBase64URLSafeString(name.getBytes(Constants.UTF_8_CHARSET));
        final String objectName = getDataSetConfigPath(projectId, datasSetId);
        try (Reader reader =
                new InputStreamReader(minioClient.getObject(fessConfig.getStorageBucket(), objectName), Constants.UTF_8_CHARSET)) {
    ...
    public void addDataSet(final String projectId, final String fileName, final InputStream in) {
        ...
        final DataSet dataSet = new DataSet();
        final String datasSetId = Base64.encodeBase64URLSafeString(fileName.getBytes(Constants.UTF_8_CHARSET));
        dataSet.setId(datasSetId);
        dataSet.setName(fileName);
        dataSet.setPath("s3://" + fessConfig.getStorageBucket() + "/" + projectFolderName + "/" + projectId + "/data/" + fileName);
        store(projectId, dataSet);

        importFiles(projectId, dataSet);
    }

    protected void importFiles(final String projectId, final DataSet dataSet) {
    ...
    private String getDataSetConfigPath(final String projectId, final String datasSetId) {
        return projectFolderName + "/" + projectId + "/config/" + datasSetId + "_dataset.json";
    }
}
