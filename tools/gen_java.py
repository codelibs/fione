...
def generate_schema(class_name, schema):
    ...
    yield " */"
    yield "package org.codelibs.fione.h2o.bindings.pojos;"
    yield ""
...
def generate_proxy(classname, endpoints):
    ...
    yield " * Copyright 2016 H2O.ai;  Apache License Version 2.0 (see LICENSE for details)"
    yield " */"
    yield "package org.codelibs.fione.h2o.bindings.proxies.retrofit;"
    yield ""
    yield "import org.codelibs.fione.h2o.bindings.pojos.*;"
...
def generate_main_class(endpoints):
    yield dedent(0, """
        ...
        import org.codelibs.fione.h2o.bindings.proxies.retrofit.*;
        import retrofit2.*;
...
def main():
    ...
    for name, schema in schemas_map.items():
        ...
        bi.write_to_file("org/codelibs/fione/h2o/bindings/pojos/%s.java" % name, generate_schema(name, schema))

    for name, values in bi.enums().items():
        bi.vprint("Generating enum: " + name)
        bi.write_to_file("org/codelibs/fione/h2o/bindings/pojos/%s.java" % name, generate_enum(name, sorted(values)))

    for name, endpoints in bi.endpoint_groups().items():
        bi.vprint("Generating proxy: " + name)
        bi.write_to_file("org/codelibs/fione/h2o/bindings/proxies/retrofit/%s.java" % name, generate_proxy(name, endpoints))

    bi.vprint("Generating H2oApi.java")
    bi.write_to_file("org/codelibs/fione/h2o/bindings/H2oApi.java", generate_main_class(bi.endpoints()))
